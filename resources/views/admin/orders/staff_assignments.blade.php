...
<div class="order-hub-page">
    <div class="header-flex d-flex flex-wrap justify-content-between align-items-center mb-4">
        <h1 class="page-title fw-bold m-0" style="color:var(--dark); font-size:24px;">{{ $title ?? 'Orders Hub' }}</h1>
        
        <div class="d-flex gap-3 align-items-center">
            <form action="{{ url()->current() }}" method="GET" class="d-flex gap-2">
                <select name="staff_id" class="form-select" style="border-radius: 50px; min-width: 200px; font-weight: 500;" onchange="this.form.submit()">
                ...
                <div class="search-box position-relative">
                    ...
                    <input type="text" name="search" class="form-control ps-5 rounded-pill" placeholder="Search..." value="{{ request('search') }}">
                </div>
            </form>
        </div>
    </div>
                ...
                <table class="table order-table m-0">
                    ...
                    <tbody>
                        ...
                        <tr id="order-row-{{ $order->id }}">
                            ...
                            <td>
                                @if(isset($order->order) && $order->order->staff)
                                    <span class="badge" style="background: rgba(67, 97, 238, 0.1); color: #4361ee; font-size: 13px; font-weight: 700; padding: 6px 12px; border-radius: 8px;">
                                        <i class="bi bi-person-check-fill me-1"></i> {{ $order->order->staff->name }}
                                    </span>
                                @else
                                    <span class="badge bg-light text-muted" style="font-size: 12px; border: 1px dashed #ccc;">Not Assigned</span>
                                @endif
                            </td>
                            <td>
