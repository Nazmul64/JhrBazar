...
<div style="background:#f3f4f6; min-height:100vh; padding:24px;">
    ...
    <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; flex-wrap:wrap;">
        ...
        <div style="background:#fff; border-radius:12px; padding:24px;
            ...
            <form action="{{ route('admin.themecolorssettings.store') }}" method="POST">
                @csrf
                ...
                <div style="margin-bottom:16px;">
                    ...
                    <input type="text" name="palette_name" value="{{ old('palette_name') }}"
                ...
                <div style="margin-bottom:16px;">
                    ...
                    <div style="display:flex; align-items:center; gap:10px;">
                        <input type="color" id="createPrimaryPicker"
                               value="{{ old('primary_color', $defaultPrimary) }}"
                        ...
                        <input type="text" name="primary_color" id="createPrimaryHex"
                               value="{{ old('primary_color', $defaultPrimary) }}"
                ...
                <div style="margin-bottom:24px;">
                    ...
                    <div style="display:flex; align-items:center; gap:10px;">
                        <input type="color" id="createSecondaryPicker"
                               value="{{ old('secondary_color', '#fbd5df') }}"
                               onchange="syncSecondary(this.value)"
                               style="width:44px; height:44px; border:2px solid #e5e7eb;
                                      border-radius:6px; cursor:pointer; padding:2px;">
                        <input type="text" name="secondary_color" id="createSecondaryHex"
                               value="{{ old('secondary_color', '#fbd5df') }}"
...
function syncPrimary(val) {
    ...
    refreshCreatePalette(val);
}
function onPrimaryHexInput(val) {
    if (/^#[0-9a-fA-F]{6}$/.test(val)) {
        document.getElementById('createPrimaryPicker').value = val;
        document.getElementById('previewPrimary').style.background = val;
        refreshCreatePalette(val);
    }
}
function syncSecondary(val) {
    document.getElementById('createSecondaryHex').value = val;
    document.getElementById('previewSecondary').style.background = val;
}
function onSecondaryHexInput(val) {
    if (/^#[0-9a-fA-F]{6}$/.test(val)) {
        document.getElementById('createSecondaryPicker').value = val;
        document.getElementById('previewSecondary').style.background = val;
    }
}

function refreshCreatePalette(hex) {
    ...
    .then(r => r.json())
    .then(data => {
        const list = document.getElementById('createPaletteList');
        list.innerHTML = '';
        ...
        // Auto-fill secondary
        document.getElementById('createSecondaryHex').value          = data.secondary;
        document.getElementById('createSecondaryPicker').value       = data.secondary;
