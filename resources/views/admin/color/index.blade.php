...
<style>
    ...
    .color-table thead tr { background:var(--surface-bg); }
    .color-table th { padding:12px 20px; font-size:13px; font-weight:600; color:var(--text-muted); border-bottom:1px solid var(--border-color); white-space:nowrap; }
    .color-table td { padding:14px 20px; font-size:14px; color:var(--text-dark); border-bottom:1px solid var(--border-color); vertical-align:middle; }
    .color-table tbody tr:last-child td { border-bottom:none; }
    .color-table tbody tr:hover { background:#fafbff; }

    .color-swatch { width:50px; height:32px; border-radius:6px; border:1px solid rgba(0,0,0,0.12); display:inline-block; }
    ...
    .modal-header { padding:18px 24px !important; border-bottom:1px solid var(--border-color) !important; background:#fff; }
    ...
    input[type="color"].color-picker {
        width:120px; height:46px; padding:4px 6px;
        border-radius:var(--radius-sm); border:1.5px solid var(--border-color);
        cursor:pointer; background:#fff;
    }
    input[type="color"].color-picker:focus { border-color:var(--brand-primary); outline:none; }
...
<div class="page-header">
    ...
    <button type="button" class="btn-add" onclick="openCreateModal()">
        <i class="bi bi-plus-circle-fill"></i> Add Color
    </button>
</div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="text-align:center;">
                        <form action="{{ route('admin.colors.toggle', $color->id) }}" method="POST" class="d-inline">
                            @csrf
                    ...
                    <td>
                        <div class="action-wrap">
                            ...
                            <button type="button" class="btn-icon btn-icon-edit"
                                    onclick="openEditModal({{ $color->id }}, '{{ addslashes($color->name) }}', '{{ $color->color_code }}')"
                                    title="Edit">
                            ...
                            <button type="button" class="btn-icon btn-icon-delete"
                                    onclick="openDeleteModal({{ $color->id }}, '{{ addslashes($color->name) }}')"
                                    title="Delete">
                ...
                <tr>
                    <td colspan="5">
                        <div class="empty-state">
                            <p class="empty-title">No Colors Found</p>
                            <p class="empty-sub">Click "Add Color" to create your first color.</p>
                        </div>
                    </td>
                </tr>
                @endforelse
        ...
        <div class="modal-content">
            ...
            <form action="{{ route('admin.colors.store') }}" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-2">
                        <label class="form-label-custom">Select Color <span>*</span></label><br>
                        <input type="color" name="color_code" id="create-color"
                               class="color-picker"
        ...
        <div class="modal-content">
            ...
            <form id="editForm" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-2">
                        <label class="form-label-custom">Select Color <span>*</span></label><br>
                        <input type="color" name="color_code" id="edit-color"
                               class="color-picker"
                               value="#000000">
                    </div>
                </div>
    ...
    <div class="modal-dialog modal-dialog-centered" style="max-width:400px;">
        <div class="modal-content">
            <div class="modal-body" style="padding:36px 24px !important; text-align:center;">
                ...
                <p style="font-size:15px; font-weight:700; color:#1a1d23; margin:0 0 8px;">Delete Color?</p>
                <p style="font-size:13.5px; color:var(--text-muted); margin:0;">
                    Are you sure you want to delete
                    <strong id="delete-color-name"></strong>?
...
<script>
    ...
    function openCreateModal() {
        ...
        document.getElementById('create-color').value = '#000000';
        new bootstrap.Modal(document.getElementById('createModal')).show();
        setTimeout(() => document.getElementById('create-name').focus(), 350);
    }

    // ── Edit Modal ────────────────────────────────────────
    function openEditModal(id, name, colorCode) {
        const url = "{{ route('admin.colors.update', ['color' => '__ID__']) }}".replace('__ID__', id);
        document.getElementById('editForm').action = url;
        document.getElementById('edit-name').value = name;
        document.getElementById('edit-color').value = colorCode || '#000000';
    ...
    function openDeleteModal(id, name) {
        const url = "{{ route('admin.colors.destroy', ['color' => '__ID__']) }}".replace('__ID__', id);
