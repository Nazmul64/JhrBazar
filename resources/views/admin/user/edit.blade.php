...
<div class="container-fluid px-4 py-4">
    ...
    <h5 class="fw-bold mb-4" style="color:#1a1a2e; font-size:20px;">Edit User</h5>
        ...
        <div class="card border-0 shadow-sm rounded-3">
            <div class="card-body p-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="user_type" id="user_type_select" class="form-select" required onchange="filterRoles()">
                                    ...
                                    @foreach($userTypes as $type)
                                        <option value="{{ $type }}" {{ old('user_type', $user->role) == $type ? 'selected' : '' }}>
                                            {{ ucfirst($type) }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="col-lg-4">
                        <div class="text-center">
                            <div id="previewContainer" class="mb-3" style="width: 200px; height: 200px; border: 2px dashed #ddd; border-radius: 12px; margin: 0 auto; display: flex; align-items: center; justify-content: center; overflow: hidden; background: #f8f9fa; cursor: pointer;" onclick="document.getElementById('profile_image').click()">
                                <img id="imagePreview" src="{{ $user->profile_image ? asset($user->profile_image) : '' }}" style="{{ $user->profile_image ? '' : 'display: none;' }} width: 100%; height: 100%; object-fit: cover;">
                                <div id="previewPlaceholder" class="text-muted" style="{{ $user->profile_image ? 'display: none;' : '' }}">
                                    <i class="fas fa-camera fa-2x mb-2"></i>
                                    <div style="font-size: 12px;">{{ $user->profile_image ? 'Change Profile' : 'Upload Profile' }}</div>
                                </div>
                            </div>
                ...
                <div class="mt-4 pt-3 border-top d-flex justify-content-end gap-2">
                    ...
                    <button type="submit" class="btn btn-primary px-5" style="background: #4f46e5; border: none;">Update User</button>
                </div>
