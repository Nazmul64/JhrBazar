...
@section('title', 'Seller Conversations')
        ...
        <div class="col-md-4">
            <div class="card shadow-sm h-100">
                <div class="card-header bg-white border-bottom py-3">
                    <h5 class="mb-0 text-primary"><i class="bi bi-shop"></i> Seller Chats</h5>
                </div>
                        ...
                        @forelse($sessions as $session)
                            <a href="#" class="list-group-item list-group-item-action d-flex align-items-center p-3 chat-session-item {{ !$session->is_read_by_admin ? 'fw-bold bg-light' : '' }}" data-session-id="{{ $session->id }}">
                                <div class="avatar bg-primary text-white rounded-circle d-flex align-items-center justify-content-center me-3" style="width: 45px; height: 45px; font-size: 20px; overflow: hidden;">
                                    @if($session->user)
                                        <img src="{{ $session->user->profile_image_url }}" alt="Seller Image" style="width: 100%; height: 100%; object-fit: cover;">
                                    @else
                                ...
                                <div class="flex-grow-1 min-width-0">
                                    ...
                                    <div class="text-muted small text-truncate">{{ $session->last_message }}</div>
                                </div>
        ...
        <div class="col-md-8">
            <div class="card shadow-sm h-100" id="chat-window-container" style="display: none;">
                <div class="card-header bg-white border-bottom py-3 d-flex justify-content-between align-items-center">
                    <h5 class="mb-0" id="chat-header-title">Select a seller</h5>
                </div>
            ...
            <div class="card shadow-sm h-100 d-flex justify-content-center align-items-center text-muted" id="chat-placeholder">
                <div class="text-center p-5">
                    ...
                    <h4 class="fw-bold text-dark">Select a Seller</h4>
                    <p class="text-muted">Click on any seller on the left to start chatting.</p>
                </div>
            </div>
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function refreshSessionList() {
            fetch('{{ route("admin.seller_chat.sessions") }}')
                .then(r => r.json())
                .then(data => {
                        ...
                        li.innerHTML = `
                            ...
                            <div class="flex-grow-1 min-width-0">
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    ...
                                    <small class="${isActive ? 'text-white-50' : 'text-muted'}">${session.time_ago}</small>
                                </div>
                            </div>
                            ${badgeHtml}
                        `;
        ...
        function loadMessages(sessionId, isPolling = false) {
            fetch(`/admin/seller-chat/${sessionId}/messages`)
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        data.data.forEach(msg => {
                            ...
                            let html = `
                                <div class="chat-message-container ${isUser ? 'align-items-start' : 'align-items-end'}">
        ...
        function handleSessionClick(e) {
            ...
            chatHeaderTitle.innerText = `Seller: ${userName}`;
        ...
        replyForm.addEventListener('submit', function(e) {
            ...
            fetch(`/admin/seller-chat/${sessionId}/reply`, {
                method: 'POST',
