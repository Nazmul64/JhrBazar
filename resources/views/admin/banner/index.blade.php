...
<style>
    ...
    .bn-table tbody tr:hover { background:#fafbff; }

    /* ── Thumbnail ── */
    .bn-thumb {
        width:160px; height:50px; object-fit:cover;
        border-radius:var(--r-sm); border:1px solid var(--border);
        display:block;
    }
    .bn-thumb-ph {
        width:160px; height:50px; background:var(--surface);
    ...
    /* ── Action buttons ── */
    .action-wrap { display:flex; align-items:center; gap:8px; justify-content:flex-end; }
    .btn-act {
    ...
    .btn-act-edit { border-color:rgba(59,130,246,.35); color:#3b82f6; }
    .btn-act-edit:hover { background:#3b82f6; border-color:#3b82f6; color:#fff; }
    .btn-act-del  { border-color:rgba(239,68,68,.35); color:#ef4444; }
...
{{-- PAGE HEADER --}}
<div class="ph">
    <h4 class="ph-title">Banner List</h4>
    <a href="{{ route('admin.banner.create') }}" class="btn-add">
        <i class="bi bi-plus-circle-fill"></i> Add Banner
    </a>
</div>
...
function openDel(id, name) {
    var base = "{{ route('admin.banner.destroy', '__ID__') }}";
