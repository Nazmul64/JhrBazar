...
<style>
    ...
    .btn-add:hover { background:var(--brand-primary-hover); transform:translateY(-1px); }
    ...
    .unit-table thead tr { background:var(--surface-bg); }
    .unit-table th { padding:12px 24px; font-size:13px; font-weight:600; color:var(--text-muted); border-bottom:1px solid var(--border-color); white-space:nowrap; }
    .unit-table td { padding:14px 24px; font-size:14px; color:var(--text-dark); border-bottom:1px solid var(--border-color); vertical-align:middle; }
    .unit-table tbody tr:last-child td { border-bottom:none; }
    .unit-table tbody tr:hover { background:#fafbff; }
        ...
        <table class="unit-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $unit->name }}</td>
                    <td>
                        <form action="{{ route('admin.units.toggle', $unit->id) }}" method="POST" class="d-inline">
                            @csrf
                    ...
                    <td>
                        <div class="action-wrap">
                            <button type="button" class="btn-icon btn-icon-edit"
                                    onclick="openEditModal({{ $unit->id }}, '{{ addslashes($unit->name) }}')"
                                    title="Edit">
                ...
                <tr>
                    <td colspan="4">
                        <div class="empty-state">
                            ...
                            <p class="empty-sub">Click "Add Unit" to create your first unit.</p>
                        </div>
                    </td>
                </tr>
                @endforelse
    ...
    <div class="modal-dialog modal-dialog-centered" style="max-width:400px;">
        <div class="modal-content">
            <div class="modal-body" style="padding:36px 24px !important; text-align:center;">
                ...
                <p style="font-size:15px; font-weight:700; color:#1a1d23; margin:0 0 8px;">Delete Unit?</p>
...
<script>
    ...
    function openEditModal(id, name) {
        const url = "{{ route('admin.units.update', ['unit' => '__ID__']) }}".replace('__ID__', id);
    ...
    function openDeleteModal(id, name) {
        const url = "{{ route('admin.units.destroy', ['unit' => '__ID__']) }}".replace('__ID__', id);
