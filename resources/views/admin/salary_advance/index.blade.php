...
<div class="container-fluid px-4 py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        <div>
            <h4 class="fw-bold mb-1" style="color:#1a1a2e; font-size:22px;">Salary & Advances</h4>
            <p class="text-muted mb-0" style="font-size:13px;">Manage salary advances and monthly payback adjustments.</p>
        </div>
        <button class="btn-hrm-add" data-bs-toggle="modal" data-bs-target="#logAdvanceModal">
            <i class="fas fa-plus me-1"></i> Log Advance Request
        </button>
    </div>
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-6 col-lg-4">
            <div class="card border-0 shadow-sm rounded-3 bg-gradient-premium text-white">
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <span class="d-block mb-1 opacity-75" style="font-size:13px; font-weight:500;">Approved Outstanding Loans</span>
                            <h3 class="fw-bold mb-0" style="font-size:26px;">৳{{ number_format($totalApprovedAmount, 2) }}</h3>
                        </div>
        ...
        <div class="col-md-6 col-lg-4">
            <div class="card border-0 shadow-sm rounded-3" style="background:#fff;">
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <span class="d-block mb-1 text-muted" style="font-size:13px; font-weight:500;">Pending Requests</span>
                            <h3 class="fw-bold mb-0" style="font-size:26px; color:#c2185b;">{{ $totalPendingRequests }}</h3>
                        </div>
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead>
                        <tr style="background:#f8f9fa;">
                            ...
                            <th class="py-3 hrm-th">Disburse Date</th>
                            <th class="py-3 hrm-th">Payback Adjustment</th>
                            <th class="py-3 hrm-th text-center">Status</th>
                            <th class="py-3 hrm-th text-center">Disbursement</th>
                    ...
                    <tbody>
                        ...
                        <tr class="hrm-row">
                            ...
                            <td>
                                <div class="d-flex align-items-center gap-2">
                                    <img src="{{ $advance->employee->profile_image ? asset($advance->employee->profile_image) : asset('admin/images/default-avatar.png') }}"
                                         class="rounded-circle" width="35" height="35" style="object-fit: cover;">
                                    <div>
                                        <div class="fw-bold" style="font-size:13.5px; color:#222;">{{ $advance->employee->name }}</div>
                                        <span class="text-muted" style="font-size:11px;">Current Basic: ৳{{ number_format($advance->employee->salary, 2) }}</span>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td>
                                <span class="badge bg-light text-dark border p-2" style="font-size:11px; display:block; width:fit-content;">
                                    {{ $advance->deduction_type }}
                                </span>
                                @if($advance->deduction_type === 'Monthly Deduct')
                                    <div class="text-muted mt-1" style="font-size:10px;">Deduct: ৳{{ number_format($advance->monthly_deduction_amount, 2) }} / Month</div>
                                @endif
                            </td>
                        ...
                        <tr>
                            <td colspan="8" class="text-center py-5">
                                ...
                                <span class="text-muted" style="font-size:14px;">No salary advance requests recorded.</span>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
    ...
    {{-- Log Advance Modal --}}
    <div class="modal fade" id="logAdvanceModal" tabindex="-1" aria-hidden="true">
            ...
            <div class="modal-content border-0 shadow-lg rounded-3">
                ...
                <form action="{{ route('admin.hrm.salary-advance.store') }}" method="POST">
                    ...
                    <div class="modal-body p-4">
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                <select name="employee_id" class="form-select hrm-in" required>
                                    ...
                                    @foreach($employees as $emp)
                                        <option value="{{ $emp->id }}">{{ $emp->name }} (৳{{ number_format($emp->salary, 2) }})</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-12">
                                <label class="hrm-lbl">Advance Amount (BDT) <span class="text-danger">*</span></label>
                                <input type="number" step="0.01" name="amount" class="form-control hrm-in" required placeholder="Enter amount">
                            </div>
                            <div class="col-md-6">
                                <label class="hrm-lbl">Request/Disburse Date <span class="text-danger">*</span></label>
                                <input type="date" name="advance_date" class="form-control hrm-in" required value="{{ date('Y-m-d') }}">
                            </div>
                            <div class="col-md-6">
                                <label class="hrm-lbl">Deduction Type <span class="text-danger">*</span></label>
                                <select name="deduction_type" id="deduction_type_select" class="form-select hrm-in" required onchange="toggleMonthlyDeduction(this.value)">
                                    <option value="Monthly Deduct">Monthly Deduct</option>
                                    <option value="One Time Deduct">One Time Deduct</option>
                                    <option value="Manual Payback">Manual Payback</option>
                                </select>
                            </div>
                            <div class="col-12" id="monthly_deduction_wrapper">
                                <label class="hrm-lbl">Monthly Installment Amount (BDT)</label>
                                <input type="number" step="0.01" name="monthly_deduction_amount" class="form-control hrm-in" placeholder="Monthly installment amount to deduct">
                            </div>
                            <div class="col-12">
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="reason" rows="3" class="form-control hrm-in" placeholder="Enter reason details..."></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer border-0 pt-0">
                        ...
                        <button type="submit" class="btn-hrm-save">Log Advance</button>
                    </div>
                </form>
            </div>
        </div>
...
.btn-hrm-action { background: #1a1a2e; color: #fff; border: none; font-size: 11px; border-radius: 6px; font-weight: 500; transition: all .2s; }
.btn-hrm-action:hover { opacity: .9; color: #fff; }
