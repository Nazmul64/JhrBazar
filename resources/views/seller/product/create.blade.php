...
<div class="container-fluid px-4 py-4">
    <div class="d-flex align-items-center gap-2 mb-4">
        <h4 class="mb-0 fw-bold" style="color:var(--dark);">Add New Product</h4>
    </div>
    ...
    <form action="{{ route('seller.product.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-card">
            ...
            <div class="row g-4">
                <div class="col-md-4">
                    ...
                    <select name="category_id" class="form-select" required>
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ old('category_id') == $category->id ? 'selected' : '' }}>{{ $category->name ?? 'Category '.$category->id }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4">
                    ...
                    <select name="sub_category_id" class="form-select">
                        ...
                        @foreach($subcategories as $sub)
                            <option value="{{ $sub->id }}" {{ old('sub_category_id') == $sub->id ? 'selected' : '' }}>{{ $sub->name ?? 'SubCategory '.$sub->id }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4">
                    ...
                    <select name="brand_id" class="form-select">
                        ...
                        @foreach($brands as $brand)
                            <option value="{{ $brand->id }}" {{ old('brand_id') == $brand->id ? 'selected' : '' }}>{{ $brand->name ?? 'Brand '.$brand->id }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <select name="color" class="form-select">
                        ...
                        @foreach($colors as $color)
                            <option value="{{ $color->name ?? '' }}" {{ old('color') == ($color->name ?? '') ? 'selected' : '' }}>{{ $color->name ?? 'Color '.$color->id }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4">
                    ...
                    <select name="unit" class="form-select">
                        ...
                        @foreach($units as $unit)
                            <option value="{{ $unit->name ?? '' }}" {{ old('unit') == ($unit->name ?? '') ? 'selected' : '' }}>{{ $unit->name ?? 'Unit '.$unit->id }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4">
                    ...
                    <select name="size" class="form-select">
                        ...
                        @foreach($sizes as $size)
                            <option value="{{ $size->name ?? '' }}" {{ old('size') == ($size->name ?? '') ? 'selected' : '' }}>{{ $size->name ?? 'Size '.$size->id }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="sku" id="sku_input" class="form-control" placeholder="Ex: 134543" value="{{ old('sku') }}">
                </div>
            </div>
        </div>
        ...
        <div class="form-card">
            ...
            <div class="row g-4">
                <div class="col-md-4">
                    ...
                    <input type="number" step="0.01" name="buying_price" class="form-control" placeholder="Buying Price" value="{{ old('buying_price') }}" required>
                </div>
                <div class="col-md-4">
                    <label class="form-label">Selling Price <span class="text-danger">*</span></label>
                    <input type="number" step="0.01" name="selling_price" class="form-control" placeholder="Selling Price" value="{{ old('selling_price') }}" required>
                </div>
                <div class="col-md-4">
                    <label class="form-label">Discount Price</label>
                    <input type="number" step="0.01" name="discount_price" class="form-control" placeholder="0" value="{{ old('discount_price', 0) }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label">Current Stock Quantity</label>
                    <input type="number" name="stock_quantity" class="form-control" placeholder="Current Stock Quantity" value="{{ old('stock_quantity') }}">
                </div>
            </div>
        </div>
        ...
        <div class="form-card">
            ...
            <div class="row g-4">
                <div class="col-md-6">
                    <label class="form-label">Thumbnail (Ratio 1:1) <span class="text-danger">*</span></label>
                    <div class="img-upload-box">
                        <input type="file" name="thumbnail" accept="image/*" required onchange="previewImg(this, 'preview1')">
                        <div id="placeholder1">
                ...
                <div class="col-md-6">
                    <label class="form-label">Gallery Images (Ratio 1:1)</label>
                    <div class="img-upload-box">
            ...
            <div class="row g-4">
                ...
                <div class="col-md-8">
                    ...
                    <div id="video_link_container" style="display: none;">
                        ...
                        <input type="url" name="video_link" class="form-control" placeholder="https://www.youtube.com/watch?v=..." value="{{ old('video_link') }}">
        ...
        <div class="form-card">
            <div class="form-title d-flex justify-content-between align-items-center">
                ...
                <div class="form-check form-switch m-0">
                    <input class="form-check-input" type="checkbox" id="seo_toggle" role="switch" onchange="toggleSEO()">
                </div>
            </div>
            <div class="row g-4" id="seo_body" style="display: none;">
                <div class="col-md-12">
                ...
                <div class="col-md-12">
                    ...
                    <input type="text" name="meta_keywords" class="form-control" placeholder="Write keywords and Press enter to add new one" value="{{ old('meta_keywords') }}">
