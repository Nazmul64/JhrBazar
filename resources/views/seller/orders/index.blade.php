...
<div class="order-hub-page">
    <div class="header-flex">
        <h1 class="page-title">My Orders Hub</h1>
        <div class="search-box">
            <i class="bi bi-search"></i>
            <form action="{{ route('seller.orders.index', $status) }}" method="GET">
    ...
    <div class="filter-pills">
        <a href="{{ route('seller.orders.index', 'all') }}" class="pill {{ $status == 'all' ? 'active' : '' }}">
            All Orders <span class="badge-count">{{ $totalOrders }}</span>
        </a>
        <a href="{{ route('seller.orders.index', 'pending') }}" class="pill {{ $status == 'pending' ? 'active' : '' }}">
            Pending <span class="badge-count">{{ $pendingOrders }}</span>
        </a>
        <a href="{{ route('seller.orders.index', 'processing') }}" class="pill {{ $status == 'processing' ? 'active' : '' }}">
            Processing <span class="badge-count">{{ $processingOrders }}</span>
        </a>
        <a href="{{ route('seller.orders.index', 'shipped') }}" class="pill {{ $status == 'shipped' ? 'active' : '' }}">
...
<script>
    ...
    function updateStatus(id, status) {
        fetch(`{{ url('seller/orders/status') }}/${id}`, {
            method: 'POST',
            headers: {
    ...
    function assignStaff(id, staffId) {
        fetch(`{{ url('seller/orders/assign-staff') }}/${id}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
    ...
    function updatePaymentStatus(id, status) {
        fetch(`{{ url('seller/orders/payment-status') }}/${id}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
    ...
    function applyBulkAction(action, forceIds = null) {
        ...
        Swal.fire({
            ...
            text: forceIds ? `Send this order to ${action}?` : `Apply action to ${selected.length} orders?`,
            icon: 'info',
            showCancelButton: true,
            confirmButtonText: 'Yes, proceed'
        }).then((result) => {
            if (result.isConfirmed) {
                fetch(`{{ route('seller.orders.bulk-action') }}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                ...
                .then(data => {
                    if(data.success) {
                        Swal.fire('Applied!', data.message, 'success').then(() => location.reload());
                    } else {
    ...
    function fetchPathaoData(type, targetId, parentId = null) {
        let url = `{{ url('seller/orders/pathao') }}/${type}`;
        if (parentId) url = `{{ url('seller/orders/pathao') }}/${type}/${parentId}`;
