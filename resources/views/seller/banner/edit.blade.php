...
<div class="container-fluid px-4 py-4">
    <div class="d-flex align-items-center gap-2 mb-4">
        ...
        <h4 class="mb-0 fw-bold" style="color:var(--dark);">Edit Banner</h4>
    </div>
    ...
    <div class="form-card" style="max-width:800px;">
        ...
        <form action="{{ route('seller.banner.update', $banner->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('POST')
            <div class="row g-4">
                <div class="col-md-12">
                    <label class="form-label">Title <span class="text-danger">*</span></label>
                    <input type="text" name="title" class="form-control" placeholder="Banner Title" value="{{ old('title', $banner->title) }}" required>
                </div>
                <div class="col-md-12">
                    <label class="form-label">Banner Image <small class="text-muted">(Leave empty to keep current image)</small></label>
                    <input type="file" name="image" class="form-control" accept="image/*" onchange="previewImg(this)">
                    <img id="preview" src="{{ $banner->image ? asset($banner->image) : '#' }}" alt="Preview" style="max-width: 100%; margin-top: 10px; display: {{ $banner->image ? 'block' : 'none' }}; border-radius: 8px;">
                </div>
            </div>
